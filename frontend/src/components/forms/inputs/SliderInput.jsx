...
export default function SliderInput({name,value,onChange,...props}){
    const {min,max,styles,step,label,inStyle} = props;
    return(
        <div className={"flex justify-between " + styles}>
            ...
            <input className={"overflow-y-hidden rounded-md w-1/3 h-4 appearance-none mr-auto " + inStyle} type="range" min={min} max={max} step={step} name={name} id={name} value={value} onChange={onChange}></input>
